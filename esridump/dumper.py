...
class EsriDumper(object):
    ...
    def _request(self, method, url, **kwargs):
        try:
            ...
            return requests.request(
                method, url, timeout=self._http_timeout, verify=False, **kwargs
            )
    ...
    def _build_query_args(self, query_args=None):
        ...
        if override_where and requested_where and requested_where != "1=1":
            ...
            override_args["where"] = "({}) AND ({})".format(
                requested_where, override_where,
            )
    ...
    def _handle_esri_errors(self, response, error_message):
        ...
        if error:
            raise EsriDownloadError(
                "{}: {} {}".format(
                    error_message, error["message"], ", ".join(error["details"]),
                )
            )
    ...
    def can_handle_pagination(self, query_fields):
        ...
        try:
            ...
            self._logger.error(
                "Could not parse response from pagination check %s as JSON:\n\n%s",
                response.request.url,
                response.text,
            )
            return False
        ...
        return (
            data.get("error") and data["error"]["message"] != "Failed to execute query."
        )

    def get_metadata(self):
        query_args = self._build_query_args({"f": "json",})
        headers = self._build_headers()
        url = self._build_url()
    ...
    def _get_layer_min_max(self, oid_field_name):
        ...
        query_args = self._build_query_args(
            {
                "where": "{} = {} OR {} = {}".format(
                    oid_field_name, min_value, oid_field_name, max_value
                ),
                "returnIdsOnly": "true",
                "f": "json",
            }
        ...
        if len(features) >= max_records:
            ...
            for child_envelope in envelopes:
                for feature in self._scrape_an_envelope(
                    child_envelope, outSR, max_records
                ):
                    yield feature
        else:
    ...
    def __iter__(self):
                ...
                try:
                    ...
                    for page_min in range(oid_min - 1, oid_max, page_size):
                        ...
                        query_args = self._build_query_args(
                            {
                                "where": "{} > {} AND {} <= {}".format(
                                    oid_field_name, page_min, oid_field_name, page_max,
                                ),
                                "geometryPrecision": self._precision,
                                "returnGeometry": self._request_geometry,
                                "outSR": self._outSR,
                        ...
                        page_args.append(query_args)
                    self._logger.info(
                        "Built {} requests using OID where clause method".format(
                            len(page_args)
                        )
                    )
        ...
        for query_args in page_args:
            ...
            if error:
                raise EsriDownloadError(
                    "Problem querying ESRI dataset with args {}. Server said: {}".format(
